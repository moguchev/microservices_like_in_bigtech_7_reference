...
func TestSocialService_DeclineFriendRequest(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:  "Positive: decline request successfully",
			argID: requestID,
			want: &models.FriendRequest{
				ID:       requestID,
				FromUser: friendUser,
			...
			mock: func(t *testing.T) social.Deps {
				...
				repoMock.EXPECT().
					UpdateFriendRequestStatus(ctx, requestID, models.FriendRequestStatusDeclined).
					Return(&models.FriendRequest{
						ID:       requestID,
						FromUser: friendUser,
						ToUser:   userID,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := uc.DeclineFriendRequest(ctx, tt.argID)
